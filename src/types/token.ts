...
function toPatreonToken(accessToken: AccessToken): PatreonToken {
  const customRefresh: RefreshFunction = async function (params?: {}): Promise<PatreonToken> {
    const refreshedAccessToken = await patreonToken.accessToken.refresh(params)
    return toPatreonToken(refreshedAccessToken)
  }

  const customRevoke: RevokeFunction = async function (tokenType): Promise<void> {
